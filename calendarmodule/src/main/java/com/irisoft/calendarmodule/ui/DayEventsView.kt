...
import com.irisoft.calendarmodule.ui.RangeView.Companion.showAsAllDay
import com.irisoft.calendarmodule.ui.utils.Day
import com.irisoft.calendarmodule.ui.utils.timeOfDay
import com.irisoft.calendarmodule.ui.utils.toCalendar
...
class DayEventsView @JvmOverloads constructor(
    ...
    private var timeCircleRadius: Int = 1
    ...
    init {
        ...
        setOnTouchListener { _, motionEvent ->
            ...
            if (motionEvent.action == MotionEvent.ACTION_UP) {
                ...
                val event = AddEvent("Title to event", "Description event", R.color.LineToday, hourToTime(hour), hourToTime(hour + 1), false)
    ...
    override fun draw(canvas: Canvas?) {
        ...
        val left = (dividerPadding - timeCircleRadius - 15).coerceAtLeast(0)
        val top = paddingTop
        val right = width - paddingRight
        val bottom = height - paddingBottom

        if (day.isToday) {
            val time = Calendar.getInstance().timeOfDay + DateUtils.HOUR_IN_MILLIS
            val posY = top + (bottom.toFloat() - top) * time / DateUtils.DAY_IN_MILLIS
            timePaint.color = ContextCompat.getColor(context, R.color.LineToday)
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        val left = dividerPadding  - 30
        val top = paddingTop
        val right = width - dividerPadding

        repeat(DAY_IN_HOURS + 10){
            divider!!.setBounds(left, (top + _hourHeight * (it + 1) ).toInt(), right, (top + _hourHeight * (it + 1) + dividerHeight).toInt())
            divider!!.draw(canvas)
        }
    }
